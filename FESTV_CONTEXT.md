...
- **framer-motion v10** (NOT v12 — v12 has a WAAPI bug that skips `initial` state on mount)
...
**Legacy HTML frontend (`/backend/legacy/`)** — Archived. Moved from `/backend/public/` to `/backend/legacy/` in April 2026.
- Plain HTML + CSS + vanilla JS — historical reference only, not served
...
- **Render Build Filters** — set to `frontend/**` AND `backend/**` so frontend-only commits trigger a redeploy. Without this, only `backend/**` changes trigger the build.
...
│   ├── EventRequestDetail.tsx ← ✅ Built — request summary, vendor card, quotes list, cancel button; CAD currency
...
↓
Within parameters → Quote auto-generated immediately (status: SENT, isAutoGenerated: true,
                    expiresAt: 7 days) + EventRequest → QUOTE_SENT + client email sent
Out of parameters → EventRequest stays PENDING, vendor creates manual Quote
↓
...
- **`search_vendors` city filter**: city is on the `User` model, not `ProviderProfile`. Query via `where.user = { city: { contains: ..., mode: 'insensitive' } }`
- **`get_price_estimate` packageId**: must be an exact UUID returned by `search_vendors`. System prompt explicitly forbids guessing.
- **Standard requests**: `create_event_request` now auto-generates a quote immediately (backend controller handles it) — Jess should tell the planner their quote is ready to view, not that they'll hear back in 24-48h.
...
32. **`AnimatePresence initial={false}` kills ALL child animations** — this prop tells framer-motion to skip initial animations for every descendant motion component, not just the route wrapper. Layout.tsx uses `<AnimatePresence mode="wait">` (no `initial={false}`). Never add it back.
