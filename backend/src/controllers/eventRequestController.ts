...
export const createEventRequest = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  // Status is PENDING_VENDOR_APPROVAL — vendor must approve before the planner can see it.
  // EventRequest stays PENDING until vendor approves (moves it to QUOTE_SENT).
  if (packageId && pricing && !isOutOfParameters) {
    await prisma.quote.create({
      data: {
        ...
        // expiresAt set when vendor approves, not now
      },
    });

    // Notify vendor that a quote is ready for their review
    const packageName = request.package?.name ?? 'your package';
    prisma.notification.create({
      data: {
        userId:  vendor.userId,
        type:    'NEW_REQUEST',
        title:   'New booking request',
        message: `${clientName} has requested ${packageName} for ${dateLabel}. Review and respond.`,
        data:    { eventRequestId: request.id },
      },
    }).catch(() => {});
  }
